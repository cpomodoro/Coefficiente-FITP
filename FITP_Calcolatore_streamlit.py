...
if uploaded and st.button("Calcola", use_container_width=True):
    ...
    try:
        st.write("Debug: inizio calcolo...")
        # Leggi direttamente dal buffer in memoria senza file temporaneo
        import pandas as pd
        file_bytes = uploaded.read()
        df_test = pd.read_excel(io.BytesIO(file_bytes), sheet_name=0, header=0)
        st.write(f"Debug: file letto, {len(df_test)} righe, colonne: {list(df_test.columns)}")

        # Salva su file temporaneo per leggi_excel
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as tmp:
            tmp.write(file_bytes)
            tmp_path = tmp.name
        st.write(f"Debug: file temporaneo creato in {tmp_path}")

        partite = leggi_excel(tmp_path, classifica, sesso, bonus_camp)
        st.write(f"Debug: lette {len(partite)} partite")
        risultato = calcola_con_promozioni(classifica, sesso, partite, bonus_camp)
        st.write(f"Debug: calcolo completato, coeff={risultato['coeff']}")
